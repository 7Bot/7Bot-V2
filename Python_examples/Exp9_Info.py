...
# 7Bot Robotic Arm Example 9: Robot Information Query (Enhanced with WebSocket Support)
...
print("Retrieving robot information...")

# 先关闭掉自动反馈，避免数据冲突
print("Disabling automatic feedback to avoid data conflicts...")
arm.setAnglesFbFreq(0)
arm.setLoadsFbFreq(0)

time.sleep(0.5)

print("\n=== Robot Information ===")
try:
    print("Device Code: ",  arm.getDeviceCode())
    print("Version: ",      arm.getVersion())
    print("MAC: ",          arm.getMAC())
    print("ID: ",           arm.getID())
    print("Offsets: ",      arm.getOffsets())
    print("Protocol: ",     PROTOCOL.upper())
    
    # Additional system information if available
    if hasattr(arm, 'getSystemInfo'):
        print("\n=== System Info ===")
        system_info = arm.getSystemInfo()
        for key, value in system_info.items():
            print(f"{key}: {value}")
            
except Exception as e:
    print(f"Error retrieving robot information: {e}")

print("\nRobot information query completed!")
